...
const addReminderToQueue = async (reminderQueue, reminder) => {
	consoleLogWithDateTime(`Adding to queue. REMINDER: ${reminder.title} - ${reminder.content}`)
	...
	console.log(await showAllRemindersInQueue(reminderQueue, "Queue after adding reminder to queue"))
}
...
const removeReminderFromQueue = async (reminderQueue, reminder) => {
	...
	for (const job of jobs) {
		if (job.data._id === reminder._id) {
			consoleLogWithDateTime(`Job ${reminder.title} found. Removing from queue.`)
			await job.remove()
		}
	}

	console.log(await showAllRemindersInQueue(reminderQueue, "Queue after removing reminder from queue."))
}
...
const checkJobState = async (reminderQueue, jobId) => {
	...
	if (jobs.length > 0) {
		for (const job of jobs) {
			if (job.id.includes(jobIdRef)) {
				consoleLogWithDateTime(`Found jobs that match ${jobIdRef} - ${job.id}`)
	...
	if (completedJobs.length === foundJobsWithJobIdRef.length) {
		...
		consoleLogWithDateTime(`Not all jobs with ${jobIdRef} are done. Removing completed jobs from queue.`)
