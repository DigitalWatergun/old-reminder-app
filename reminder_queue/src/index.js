...
const messageHandler = (message) => {
	...
	consoleLogWithDateTime(`Received message ${message.id} for ${reminder.title} with the event ${attributes.eventType}`)
...
const subscription = pubSubClient.subscription(process.env.GCP_PUBSUB_SUBSCRIPTION_ID)

consoleLogWithDateTime("Current Job Queue:")
console.log(await showAllRemindersInQueue(reminderQueue, "Deployment startup"))
...
reminderQueue.on("global:completed", async (jobId) => {
	...
	console.log(await showAllRemindersInQueue(reminderQueue, "Queue after completed"))
