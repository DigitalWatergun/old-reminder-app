...
const validateUserRegister = body => {
    const result = {}
    switch (true) {
        case !("username" in body) || !("password" in body) || !("confirmPassword" in body) || !("email" in body): 
...
const validateChangePassword = body => {
    ...
    console.log(body)
    switch(true) {
        case !("currentPassword" in body) || !("newPassword" in body) || !("confirmNewPassword" in body): 
            result["error"] = "There can be no blank fields"
            result["status"] = false
            break
